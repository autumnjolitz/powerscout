...
YEAR_2000_OFFSET = (
    datetime.datetime(2000, 1, 1) - datetime.datetime.utcfromtimestamp(0)
).total_seconds()

logger = logging.getLogger(__name__)
eagle = Blueprint("eagle")
...
@eagle.route("/fastpoll/<mac_id>/<seconds>", name="fastpoll_by_seconds")
def fastpoll(request, mac_id, seconds=4):
    ...
    if not request.redis.sismember("meters", "meter-{}".format(mac_id)):
        return as_json({"code": -1, "message": f"{mac_id} is not recognized"}, 400)
    try:
        eagle_id = request.redis.hget("eagles", mac_id).decode("ascii")
    except AttributeError:
        return as_json({"code": -2, "message": "Eagle mapping not ready. Cannot reverse."}, 400)
    request.redis.rpush(f"{eagle_id}-commands", f"fastpoll|{seconds}")
    return as_json({"code": 0, "message": "Ok"})
...
def consume(request):
    ...
    try:
        ...
        body = {element.tag: {leaf.tag: leaf.text for leaf in element} for element in root}
    except Exception:
    ...
    if "InstantaneousDemand" in body:
        ...
        instant_demand = (
            int(item["Demand"], 16)
            * (int(item["Multiplier"], 16) or 1)
            / (int(item["Divisor"], 16) or 1)
        )
        ...
        post_metric(f"meters.{name}.instant_demand.watts", instant_demand * 1000.0, timestamp_utc)
        post_metric(
            f"meters.{name}.instant_demand.tx_info.delay.eagle.device",
            eagle_timestamp_utc - timestamp_utc,
        )
        post_metric(
            f"meters.{name}.instant_demand.tx_info.delay.server.eagle",
            utc_now - eagle_timestamp_utc,
        )
        post_metric(
            f"meters.{name}.instant_demand.tx_info.delay.server.device", utc_now - timestamp_utc
        )
    ...
    if "CurrentSummationDelivered" in body:
        ...
        post_metric(
            f"meters.{name}.current_sum.tx_info.delay.eagle.device",
            eagle_timestamp_utc - timestamp_utc,
        )
        post_metric(
            f"meters.{name}.current_sum.tx_info.delay.server.eagle", utc_now - eagle_timestamp_utc
        )
        post_metric(
            f"meters.{name}.current_sum.tx_info.delay.server.device", utc_now - timestamp_utc
        )
    ...
    queue = []
    commands = {
        command: args for command, args in (x.decode("ascii").split("|", 1) for x in commands)
    }
