...
def check_status(session, status_url=None):
    ...
    post_endpoint = "{APC_ENDPOINT}{}".format(soup.find("form").attrs["action"], **config.config)
...
async def update_apc_status():
    ...
    async with aiohttp.ClientSession():
        ...
        *_, capacity, life_mins_raw = data.find_all("td")
        ...
        vac = float(vac)
        battery_life_status = (
            ups_status.find("span", {"id": "langBatteryLifeStatus"})
        ...
        for outlet in outlet_status.find_all("td", {"class": "dataName"}):
            ...
            for char in value:
                ...
                queue.append(char)
            if queue:
                outlet_loads[outlet_name] = {"value": float("".join(queue)), "unit": "watts"}
    status = "ok"
    if vac < 110:
        status = "warning"
        if capacity < 50:
    ...
    ups_status = {
        ...
        "capacity": capacity,
        "time_left": {"value": life_value, "unit": life_span_metric},
        "load": outlet_loads,
    }

    db.hmset("apc_status", {"data": _json.dumps(ups_status), "timestamp": time.time()})
